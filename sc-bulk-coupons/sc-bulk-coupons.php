...
# The folloeing function is now reduntant leaving it here for now incase we want to reuse it # 30/10/25

	/**
 * Generate one or two CSVs depending on mode.
 *
 * @param string $mode 'full' | 'codes' | 'both'
 * @param array  $rows Array of associative arrays (what you already export today).
 * @return array{files: array<string>, messages: array<string>} Files are absolute paths.
 */
	function scbc_generate_by_mode( $mode, array $rows ) {
		$mode = in_array( $mode, array( 'full', 'codes', 'both' ), true ) ? $mode : 'full';
		...
		$out = array( 'files' => array(), 'messages' => array() );

		// helper to write a CSV (headers inferred from row keys)
		$write_csv = static function( array $rows_in, $suffix ) use ( $basedir, $ts ) {
			$filename  = "surecart-promo-codes-{$suffix}-{$ts}.csv"; // keeps your naming + adds 'full'/'codes'
			$full_path = $basedir . $filename;

			$headers = array();
			if ( ! empty( $rows_in ) && is_array( $rows_in[0] ) ) {
				$headers = array_keys( $rows_in[0] );
			}

			$fh = fopen( $full_path, 'w' );
			if ( false === $fh ) {
				return array( 'path' => null, 'error' => 'Could not open CSV for writing.' );
			}
			...
			fclose( $fh );
			return array( 'path' => $full_path, 'error' => null );
		};
		...
		$codes_rows = array();
		foreach ( $rows as $r ) {
			$code = isset( $r['code'] ) ? $r['code'] : ( isset( $r['coupon_code'] ) ? $r['coupon_code'] : '' );
			$codes_rows[] = array( 'code' => $code );
		}

		// Write according to mode
		if ( 'full' === $mode || 'both' === $mode ) {
			$res = $write_csv( $full_rows, 'full' );
			if ( $res['error'] ) {
				$out['messages'][] = 'Export failed (full): ' . esc_html( $res['error'] );
			} else {
				$out['files'][]    = $res['path'];
				$out['messages'][] = 'Full CSV generated.';
			}
		}

		if ( 'codes' === $mode || 'both' === $mode ) {
			$res = $write_csv( $codes_rows, 'codes' );
			if ( $res['error'] ) {
				$out['messages'][] = 'Export failed (codes): ' . esc_html( $res['error'] );
			} else {
				$out['files'][]    = $res['path'];
				$out['messages'][] = 'Codes-only CSV generated.';
			}
		}
